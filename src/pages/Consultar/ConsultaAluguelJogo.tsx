...
const ConsultaAluguelJogo: React.FC = () => {
  ...
  const handleBuscar = async () => {
    ...
    try {
      const url = `api/acmegames/jogo/aluguel/${numero}`;
      const response = await axios.get<Aluguel[]>(url);
      const alugueisArray = response.data;

      if (alugueisArray.length > 0) {
        setAluguel(alugueisArray[0]);
      } else {
  ...
  return (
    <Paper elevation={3} sx={{ padding: 4, maxWidth: 900, margin: 'auto' }}>
      <Typography variant="h5" gutterBottom>
        Consulta de Aluguel por Número (Jogo)
      </Typography>
...
export default ConsultaAluguelJogo;
